...
describe('Liquality wallet SEND feature', async () => {
  ...
  it('Send SOV to random ETH address', async () => {
    ...
    let sentAmount = await page.$eval('#transaction_detail_sent_amount', el => el.innerText)
    expect(sentAmount.toString().trim()).equals('1 SOV')
    await page.waitForSelector('#transaction_details_send_to_link', { visible: true })
    let sedToHrefLink = await page.$eval('#transaction_details_send_to_link', el => el.href)
    ...
    let sendNetworkSpeedFee = await page.$eval('#transaction_details_network_speed_fee', el => el.innerText)
    ...
    let transactionIdHrefLink = await page.$eval('#transactionLink', el => el.href)
