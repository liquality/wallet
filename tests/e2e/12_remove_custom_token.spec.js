...
if (process.env.NODE_ENV === 'mainnet') {
  console.log('tests i nremove token')
  describe('Create new wallet - Remove custom tokens only against Mainnet', async () => {
    console.log('tests heren')
    beforeEach(async () => {
      console.log('tests before each')
    ...
    it('ETHEREUM - [Tether USD]', async () => {
      ...
      expect(await page.$eval(`#${symbol}_toggle_button > label`, el => el.getAttribute('class')),
        'Custom token is removed').not.contains('vue-js-switch')
    })
