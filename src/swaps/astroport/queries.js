...
export const getRateNativeToAsset = (fromAmount, asset, pairAddress) => {
  ...
  const query = {
    simulation: {
      offer_asset: {
        amount: fromAmount,
        ...(isDenom && {              // Apply this query when we do NATIVE <-> ERC20 
          info: {
            native_token: {
              denom: asset
            }
        ...
        ...(!isDenom && {             // Apply this query when we do ERC20 <-> NATIVE
          info: {
            token: {
              contract_addr: asset
            }
      ...
      operations: [
        {
          astro_swap:
          {
            ...(isFirstAssetDenom && {               // Apply this query when we do LUNA <-> ERC20
              offer_asset_info: {
                native_token: {
                  denom: firstAsset
                }
            ...
            ...(!isFirstAssetDenom && {             // Apply This query when we do ERC20 <-> ERC20
              offer_asset_info:
              {
                token: {
        ...
        },
        {
          astro_swap:
          {
            ...
            ...(isSecondAssetDenom && {          // Apply this query when we do ERC20 <-> LUNA
              ask_asset_info: {
                native_token: {
                  denom: secondAsset
                }
            ...
            ...(!isSecondAssetDenom && {        // Apply this query when we do ERC20 <-> ERC20
              ask_asset_info:
              {
                token: {
