<template>
  <div class="uniswap-swap-details">
    ...
    <div class="row" v-if="item.receiveTxHash"><div class="col">Receive Transaction: <a :href="getExplorerLink(item.receiveTxHash)" target="_blank">{{ item.receiveTxHash }}</a></div></div>
  </div>
</template>
