...
function createBtcClient (network, mnemonic, walletType, derivationPath) {
...
function createEthClient (asset, network, mnemonic, walletType, derivationPath) {
  const isTestnet = network === 'testnet'
  ...
  return createEthereumClient(asset, network, ethereumNetwork, infuraApi, scraperApi, feeProvider, mnemonic, walletType, derivationPath)
}
...
function createRskClient (asset, network, mnemonic, walletType, derivationPath) {
  const isTestnet = network === 'testnet'
  ...
  return createEthereumClient(asset, network, rskNetwork, rpcApi, scraperApi, feeProvider, mnemonic, walletType, derivationPath)
}
...
export const createClient = (asset, network, mnemonic, walletType, derivationPath) => {
  const assetData = cryptoassets[asset]

  if (assetData.chain === 'bitcoin') return createBtcClient(network, mnemonic, walletType, derivationPath)
  if (assetData.chain === 'rsk') return createRskClient(asset, network, mnemonic, walletType, derivationPath)
  ...
  return createEthClient(asset, network, mnemonic, walletType, derivationPath)
}
