...
test('get bitcoin derivation path for ledger account type Segwit', () => {
  expect(getDerivationPath(ChainId.Bitcoin, 'mainnet', 0, 'bitcoin_ledger_nagive_segwit')).toBe(
    "84'/0'/0'"
  )
})

test('get bitcoin derivation path for ledger account type Legacy', () => {
  expect(getDerivationPath(ChainId.Bitcoin, 'mainnet', 0, 'bitcoin_ledger_legacy')).toBe(
    "44'/0'/0'"
  )
})
...
test('get RSK derivation path for Ledger accounts on testnet', () => {
  expect(getDerivationPath(ChainId.Rootstock, 'testnet', 0, 'rsk_ledger')).toBe(
    "m/44'/37310'/0'/0/0"
  )
})
