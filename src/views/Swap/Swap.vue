<template>
  <div class="view-container">
    <div class="swap" v-if="currentStep === 'inputs'">
      ...
      <InfoNotification v-if="showNoLiquidityMessage && sendAmount >= min && sendAmount > 0">
        <NoLiquidityMessage :isPairAvailable="isPairAvailable" :asset="toAsset" />
      </InfoNotification>
      <InfoNotification
        v-else-if="
          canCoverNetworkFee && sendAmount >= min && sendAmount > 0 && !insufficientFundsError
        "
      >
      ...
      <InfoNotification v-else-if="showBridgeAssetDisabledMessage && !insufficientFundsError">
...
export default {
  ...
  methods: {
    ...
    _updateQuotes: _.debounce(async function () {
      ...
      // if(this.cannotCoverMinimum) this.sendAmount = 0
